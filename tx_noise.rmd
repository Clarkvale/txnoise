...
#loading in data
kinetics_noise <- read.delim("data/ln_nas_linked_kinetics_noise.bed")
...
data <- kinetics_noise %>% select("k_on", "bs", "Ntot", "Nint", "Next") %>% log10()
...
ggplot(kinetics_noise, aes(factor(string.Freq), Nint)) +
  geom_violin(scale = "width", linewidth = 1) +
  geom_dotplot(binaxis = "y", binwidth = 0.015,  mapping = aes(alpha = factor(string.Freq)), color = "blue") +
  scale_alpha_manual(values = c(0, seq(from = 0.2, to = 1 , by = 0.9/20))) +
  scale_y_log10() +
  geom_smooth(data = kinetics_noise, mapping= aes(Freq, Nint), color = "red") +
  coord_cartesian(xlim = c(0,11)) +
  theme_minimal(base_size = 18) +
  theme(legend.position = "none") +
  ylab("Intrinsic Noise") +
...
interact_plot(nint_model, pred = k_on, modx = Freq, modx.values = c(1:10), colors = q10, legend.main = "# of Enhancers") + theme_minimal(base_size = 18) +
  xlab("Burst Frequency") +
  ylab("Intrinsic Noise") +
  scale_x_log10()

interact_plot(nint_model, pred = bs, modx = Freq, modx.values = c(1:10), colors = q10, legend.main = "# of Enhancers") + theme_minimal(base_size = 18) +
  xlab("Burst Size") +
  ylab("Intrinsic Noise") +
  scale_x_log10()
```
